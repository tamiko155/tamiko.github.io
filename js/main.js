jQuery(document).ready(function() {
    ...
    for (var t = jQuery(".home .cvitae-main-nav.scroll li").children(), a = [], r = 0; r < t.length; r++) {
        var i = t[r],
    ...
    jQuery(window).scroll(function() {
        for (var e = jQuery(window).scrollTop(), t = jQuery(window).height(), r = jQuery(document).height(), i = 0; i < a.length; i++) {
            ...
            if ("#" != !o[0]) {
                var n = jQuery(o).offset().top - 100,
                    s = jQuery(o).height();
                e >= n && n + s > e ? jQuery("a[href='" + o + "']").addClass("active") : jQuery("a[href='" + o + "']").removeClass("active")
            }
        }
        if (e + t == r && !jQuery("nav li:last-child a").hasClass("active")) {
            var c = jQuery(".active").attr("href");
            jQuery("a[href='" + c + "']").removeClass("active"), jQuery("nav li:last-child a").addClass("active")
        }
        ...
        jQuery("#achievement").parallax("50%", .6), jQuery("#testimonial").parallax("50%", .6), jQuery("#clients").parallax("50%", .6), jQuery("nav.cvitae-main-nav").waypoint({
            element: this,
            handler: function(e) {
                "down" == e ? ($(".cvitae-menu-wrapper").addClass("sticky"), $(".cvitae-main-nav").css({
                    width: $(".cvitae-site-container").width()
                })) : ($(".cvitae-menu-wrapper").removeClass("sticky"), $(".cvitae-main-nav").css({
                    width: "100%"
                }))
            },
            offset: "-150"
        ...
        handler: function(e) {
            jQuery(".progress-bar").each(function() {
                ...
                jQuery(this).find(".bar").animate({
                    width: e + "%"
                }, 5e3, "swing"), jQuery(this).find(".text").animate({
                    left: e + "%"
                }, {
                    duration: 5e3,
                    easing: "swing",
        ...
        jQuery("#cvitae-slider").catslider(), jQuery(".cvitae-gallery .project").on("click", function(e) {
            e.preventDefault();
            var t = jQuery(this);
            jQuery("#project-title").text(t.data("project-title")), jQuery("#project-image").attr("src", t.data("project-image")), jQuery("#project-link").attr("href", t.data("project-link")), jQuery("#project-detail").text(t.data("project-detail")), jQuery(".cvitae-portfolio-popup").addClass("active")
        })
